...
class Scene1(Scene):
    def __init__(self, manager):
        ...
        self.button1 = Button(
            ...
            RED, BLUE, "Black", BLACK, WHITE, self.to_scene2
        )
        self.button2 = Button(
    ...
    def to_scene2(self):
        self.manager.switch_scene(Scene2(self.manager))
...
class Scene2(Scene):
    ...
    def back_to_scene1(self):
        self.manager.switch_scene(Scene1(self.manager))
...
def main():
    ...
    scene_manager.switch_scene(Scene1(scene_manager)) # start scene 1
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT: # exit event
                pygame.quit()
                sys.exit()
            scene_manager.handle_event(event) # any other events

        # continuously (beacasue we are in a while true loop) update the scene, draw the scene, and rerender the display at 30fps
