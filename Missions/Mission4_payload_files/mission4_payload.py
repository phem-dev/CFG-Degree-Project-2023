...
class GamePlay:
        ...
        for i in range(4):
            for j in range(4):
                ...
                if i * 4 + j in self.tetromino.display_tetrominoes():
                    ...
                    if (
                        i + self.tetromino.y > self.height - 1
                        or j + self.tetromino.x > self.width - 1
                        or j + self.tetromino.x < 0
                        or self.field[i + self.tetromino.y][j + self.tetromino.x] > 0
                    ):
                        intersects = True
        return intersects
    ...
    def halt_tetromino(self):
        ...
        for i in range(4):
            for j in range(4):
                if i * 4 + j in self.tetromino.display_tetrominoes():
                    self.field[i + self.tetromino.y][
                        j + self.tetromino.x
                    ] = self.tetromino.colour
        self.delete_line()
